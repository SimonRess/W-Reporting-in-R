---
#Fix error "Tool(s) not installed or not in PATH: ghostcript" -> set environment variable (https://rnbeads.org/data/installing_rnbeads.html)
author: Simon Ress
institute: Ruhr-Universität Bochum
title: Workshop Web-Scraping in R
subtitle: Modern Causal Analysis. Rubin Causal Model und Directed Acyclic Graphs
date: 15.02.2020
header-includes:
output:
  beamer_presentation:
    keep_tex: true
    toc: true #include table of contents 
    theme: Goettingen
    includes:
      in_header: 
        - styles/head.tex #for numbered sections
          #content of head.tex:
            #\setbeamertemplate{frametitle}{\thesection \hspace{0.2cm} \insertframetitle}
            #\setbeamertemplate{section in toc}[sections numbered]
            #\setbeamertemplate{subsection in toc}[subsections numbered]
        - styles/pagenumbers.tex
          #\setbeamertemplate{page number in head/foot}[totalframenumber]
  html_document: default
  word_document: default
  pdf_document: default
---
```{r options}

```
# Wrap long lines <!-- (New section) -->

## test

```{r picture, out.width=250, echo=FALSE, title="aa"}
#knitr::caption{Schema von Web-Scraping \& zentrale Befehle}
knitr::include_graphics("figure/overview-web-scraping.png")
#knitr::footnotesize{Quelle: https://github.com/yusuzech/r-web-scraping-cheat-sheet}
```


## First
Long lines in the output will not be (hard-)wrapped automatically unless you break them manually.<br>
a `html` a

```{r long_output}
a <- '## "stx2A; shiga-like toxin II A subunit encoded by bacteriophage BP-933W; K11006 shiga toxin subunit A" '
a
```

Perhaps you see it is wrapped in RStudio, but that is because `<pre>` has the CSS attribute `white-space: pre-wrap;`. For LaTeX output, the output is in the `verbatim` environment by default, which will not wrap long lines (everything is shown as is).

## Second

You can redefine the output hook function so that you can instruct some chunks to hard-wrap their output lines, e.g.

```{r wrap-hook}
library(knitr)
hook_output = knit_hooks$get('output')
knit_hooks$set(output = function(x, options) {
  # this hook is used only when the linewidth option is not NULL
  if (!is.null(n <- options$linewidth)) {
    x = knitr:::split_lines(x)
    # any lines wider than n should be wrapped
    if (any(nchar(x) > n)) x = strwrap(x, width = n)
    x = paste(x, collapse = '\n')
  }
  hook_output(x, options)
})
```

Now see the output of `a` (note `linewidth` is not a default **knitr** option):

```{r test, linewidth=60}
a
```